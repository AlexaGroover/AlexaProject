...
const WhatIsMyMailIdHandler = {
	canHandle(handlerInput) {
		return handlerInput.requestEnvelope.request.type === 'IntentRequest' &&
			handlerInput.requestEnvelope.request.intent.name === 'WhatIsMyMailId';
	}, 
	handle(handlerInput) {
		const { accessToken } = handlerInput.requestEnvelope.session.user;
		if(accessToken === null || accessToken === undefined) {
			return handlerInput.responseBuilder
				.speak('You need to Login to access further details')
				.withLinkAccountCard()
				.getResponse();
		} else {
			return new Promise((resolve) => {
				getEmailAddress(accessToken, (response) => {
					var speechText = 'Error occurred to retrieve data';
					if(response) {
						speechText = `Hey ${response.name}, Your Mail Id is ${response.email}`;
					}
...
function getEmailAddress(token, callbackFunc) {
	...
	var req = https.request(requestOptions, res => {
		...
		res.on('end', () => {
			callbackFunc(JSON.parse(returnData));
		});
	});
